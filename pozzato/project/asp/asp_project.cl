...
:- assegna(partita(S1, S2), G1), assegna(partita(S2, S1), G2), G1 < G2, G2 - G1 < 6.
...
% Prova con 12 squadre
% squadra(napoli; milan; inter; juventus; atalanta; roma; lazio; fiorentina; sassuolo; torino; udinese; bologna).
% in(napoli, napoli).
% in(milan, milano).
% in(inter, milano).
% in(juventus, juventus).
...
% 1 {assegna(partita(S1, S2), G) : giornata(G)} 1 :- partita(S1, S2).
% 11 {partita(S1, S2) : squadra(S2), S1 <> S2} 11 :- squadra(S1).
% 6 {assegna(partita(S1, S2), G) : partita(S1,S2)} 6 :- giornata(G).
% :- assegna(partita(S1, S2), G), assegna(partita(S1, S3), G), S2 <> S3.
% :- assegna(partita(S2, S1), G), assegna(partita(S3, S1), G), S2 <> S3.
% :- assegna(partita(S1, _), G), assegna(partita(_, S1), G).
% :- assegna(partita(S1, _), G), assegna(partita(S2, _), G), in(S1, C), in(S2, C), S1 <> S2.
...
% assegnamenti iniziali per velocizzare il calcolo
% prima giornata
...
% 10 assegnamenti totali per squadra Milan (1 giornata iniziale + 9 successive)
